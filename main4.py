            ...
            if message['answer']:  # 답변이 있다면
                ...
                if 'feedback' in message:
                    ...
                    if start_idx != -1:
                        ...
                        # 코드 블록 감지
                        has_code = ('Copy' in section or 
                                   'class ' in section or 
                                   'def ' in section or
                                   'lambda ' in section or
                                   '=' in section or  
                                   'print(' in section or  
                                   'import ' in section or  
                                   'random.' in section or
                                   'len(' in section or
                                   '[' in section)
                        
                        # 디버깅을 위해 has_code 값 출력
                        st.write(f"DEBUG - Has code: {has_code}")
                        
                        # 코드 블록이 있는 경우에만 자세히 보기 버튼 표시
                        if has_code:
                            if st.button("💻 자세히 보기", key=f"example_{i}"):
                                # Copy 이후의 코드 블록 찾기
                                parts = section.split('Copy')
                                for part in parts[1:]:
                                    code = part.strip()

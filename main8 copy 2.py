    ...
    for i, message in enumerate(st.session_state["messages"]):
        ...
        if message['answer']:
            ...
            if 'feedback' not in message:
                ...
                # 질문과 관련된 PDF 내용을 찾습니다
                docs = retriever.get_relevant_documents(message['question'])
                
                if docs:  # 관련 내용을 찾았다면
                    # 첫 번째 관련 문서의 내용을 가져옵니다
                    context = docs[0].page_content
                    # AI로 피드백을 생성합니다
                    feedback_chain = feedback_prompt | ChatOpenAI(
                        model="gpt-3.5-turbo-0125",  # 사용할 AI 모델
                        temperature=0.2  # 답변의 창의성 정도 (0에 가까울수록 일관된 답변)
                    ) | StrOutputParser()
                    
                    # AI에게 피드백을 요청합니다
                    feedback = feedback_chain.invoke({
                        "context": context,  # PDF에서 찾은 관련 내용
                        "question": message['question'],  # 면접 질문
                        "answer": message['answer']  # 사용자의 답변
                    })
                    ...
                    # 화면을 새로고침하여 피드백을 표시합니다
                    st.rerun()
            
            # 피드백이 있는 경우 화면에 표시합니다
            if 'feedback' in message:
                ...
                # 관련 내용을 추출합니다
                if start_idx != -1:
                    # 다음 질문 시작 부분을 찾아서 현재 질문 관련 내용만 추출
